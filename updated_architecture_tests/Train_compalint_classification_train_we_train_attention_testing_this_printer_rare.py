...
num_cat = len(lables[0])
count = [0]*num_cat

for i in range(len(lables)):
    count[np.argmax(lables[i])] += 1
...
for i in range(len(dataset_t)):
	...
	if complaint_lable_index_map[dataset_t['category_name'][i]] in  freq_cat:
        	continue
	...
	if complaint_lable_index_map[dataset_t['category_name'][i]] in probs_predicted[0].argsort()[-5:]:
        	c5 += 1
	if i%100 == 0:
		print(c1/float(co)," : ",c2/float(co)," : ",c3/float(co)," : ",c5/float(co))
